...
const Component: FC<PopoverProps> = memo(
  ({ className, interactionType, isOpen, handleChange, ...restProps }) => {
    const openStateData = useState(!!isOpen);
    const [openState, setOpenState] = openStateData;

    const handleMouseOver = useCallback(
      () => interactionType === 'hover' && setOpenState(true),
      [interactionType, setOpenState]
    );
    const handleMouseLeave = useCallback(
      () => interactionType === 'hover' && setOpenState(false),
      [interactionType, setOpenState]
    );
