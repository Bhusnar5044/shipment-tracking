...
const OptionContainer: FC<OptionContainerProps> = memo(
  ({ isOpen, options, isLabelExist, handleOptionClick, value, handleNotFoundOptionClick }) => {
        ...
        {options?.length ? (
          <List className="list-none" height={height} itemCount={options.length} itemSize={40} width={"100%"}>
            {({ index, style }) => (
              <option
                className={cn(
                  "decoration text-neutral-800 w-full cursor-pointer px-3 py-1 text-left text-black hover:bg-gray-200",
                  {
          ...
          <option
            key={"not-found"}
            className={cn("cursor-pointer px-3 py-1 text-neutral-800")}
            onClick={handleNotFoundOptionClick}
          >
            No Matches Found
          </option>
        )}
